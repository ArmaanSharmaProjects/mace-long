...
class MACECalculator(Calculator):
        ...
        if model_type in ["MACE", "EnergyDipoleMACE"]:
            self.implemented_properties.extend(
                [
                    "energy",
                    "e_elec",           # Add this
                    "e_elec_tensor",    # Add this
                    "positions_tensor", # Add this
                    "energies",
                    "free_energy",
                    "node_energy",
                    "forces",
    ...
    def calculate(self, atoms=None, properties=None, system_changes=all_changes):
        """
        Calculate properties for 4G-MACE.
        Ensures e_elec_tensor and positions_tensor are preserved for analytic forces.
        """
        # 1. Standard ASE setup
        ...
        # 2. Loop over models in the committee
        for i, model in enumerate(self.models):
            batch = self._clone_batch(batch_base)
            
            # CRITICAL: Enable gradients for positions
            batch["positions"].requires_grad_(True)
            
            live_positions_list.append(batch["positions"])
            ...
            # [FIX] Only append if it is a Tensor. Standard MACE returns float 0.0
            if "e_elec" in out and isinstance(out["e_elec"], torch.Tensor):
                live_e_elec_list.append(out["e_elec"])
        ...
        # 4. PRESERVE LIVE TENSORS
        # [FIX] Check list length to prevent crash on Original MACE (where list is empty)
        if len(live_e_elec_list) > 0:
             e_elec_stack = torch.stack(live_e_elec_list)
             self.results["e_elec_tensor"] = torch.mean(e_elec_stack, dim=0) * self.energy_units_to_eV
             self.results["positions_tensor"] = live_positions_list[0]

        # 5. Standard ASE property conversion loop
        scalar_tensors = set(["energy", "e_elec"])
        results_store_ensemble = set(["energy", "forces", "stress", "dipole", "e_elec"])
        
        for results_key, ret_key, unit_conv in [
            ("energy", "energy", self.energy_units_to_eV),
            ("e_elec", "e_elec", self.energy_units_to_eV), 
            ...
            if ret_tensors.get(ret_key) is not None:
                data_tensor = torch.mean(ret_tensors[ret_key], dim=0)
                
                if ret_key in scalar_tensors:
                    self.results[results_key] = data_tensor.cpu().item() * unit_conv
                else:
                    self.results[results_key] = data_tensor.cpu().numpy() * unit_conv

                if self.num_models > 1 and results_key in results_store_ensemble:
                    self.results[results_key + "_comm"] = ret_tensors[ret_key].cpu().numpy() * unit_conv
                    self.results[results_key + "_var"] = torch.var(ret_tensors[ret_key], dim=0).cpu().numpy() * unit_conv

        # 6. Final ASE formatting
