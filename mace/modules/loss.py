...
def mean_squared_error_charges(
    ...
    # Flatten shapes to (N_atoms,)
    # We use .view(-1) to ensure we compare 1D vectors even if dimensions differ slightly
    ref_q = ref["charges"].view(-1)
    pred_q = pred["charges"].view(-1)
    
    # Calculate MSE
    raw_loss = configs_weight * torch.square(ref_q - pred_q)
    return reduce_loss(raw_loss, ddp)
...
class WeightedEnergyForcesLoss(torch.nn.Module):
    def __init__(self, energy_weight=1.0, forces_weight=1.0, charges_weight=0.0) -> None:
        super().__init__()
    ...
    def forward(
        ...
        loss = self.energy_weight * loss_energy + self.forces_weight * loss_forces
        
        # [NEW] Add Charge Loss conditionally
        if self.charges_weight > 1e-6:
