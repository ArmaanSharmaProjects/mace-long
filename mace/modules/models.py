...
from mace.modules.electrostatics import ElectronegativityModel, QeqSolver, GaussianSmearing
...
class MACE(torch.nn.Module):
    def __init__(
        ...
        if self.use_electrostatics:
            # 1. Determine Input Dimension for Chi (Scalars only)
            # hidden_irreps is an o3.Irreps object. We count how many (0, 1) scalars there are.
            chi_in_dim = hidden_irreps.count(o3.Irrep(0, 1))
            
            # 2. Initialize Electronegativity Model (The "Head")
            self.chi_net = ElectronegativityModel(in_features=chi_in_dim)
            
            # 3. Initialize Qeq Solver
            self.qeq = QeqSolver(hardness_init=hardness_init, covalent_radii=covalent_radii)
            
            # 4. Initialize Charge Encoder (Gaussian Smearing)
            # This expands the scalar charge into a vector 
            n_charge_basis = 8
            self.charge_encoder = GaussianSmearing(num_gaussians=n_charge_basis)
            
            # 5. Initialize Projection Layer
        
            self.charge_projection = torch.nn.Linear(chi_in_dim + n_charge_basis, chi_in_dim)
    ...
    def forward(
        ...
        if self.use_electrostatics:
             total_energy = total_energy + e_elec
