...
def get_loss_fn(
    ...
    charges_weight = getattr(args, "charges_weight", 0.0) if getattr(args, "use_electrostatics", False) else 0.0
    ...
    if args.loss == "weighted":
        ...
        loss_fn = modules.WeightedEnergyForcesStressLoss(
            ...
            charges_weight=charges_weight  # <--- Can also add here if implementing stress+charges
        )
        ...
        loss_fn = modules.WeightedEnergyForcesLoss(
            energy_weight=1.0, 
            forces_weight=1.0,
            charges_weight=charges_weight # <--- Ensure fallback gets it too
        )
    return loss_fn
