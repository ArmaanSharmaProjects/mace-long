...
def main() -> None:
    ...
    if hasattr(args, "config") and args.config is not None:
        try:
            ...
            # Inject dictionaries if they exist in yaml
            if "hardness_init" in yaml_config:
                # vars(args) lets us assign new attributes that weren't in argparse
                vars(args)["hardness_init"] = yaml_config["hardness_init"]
                logging.info(f"Loaded hardness_init for {len(yaml_config['hardness_init'])} elements.")
            
            if "covalent_radii" in yaml_config:
                vars(args)["covalent_radii"] = yaml_config["covalent_radii"]
                logging.info(f"Loaded covalent_radii for {len(yaml_config['covalent_radii'])} elements.")
                
            # Also ensure weights are carried over if not set via CLI
            if "charges_weight" in yaml_config:
                vars(args)["charges_weight"] = float(yaml_config["charges_weight"])
                
        except Exception as e:
            logging.warning(f"Failed to load extra electrostatics config: {e}")
